...
import { CheckCircle, Search, AlertTriangle, Trash2, Edit, Plus, UploadCloud, X, RotateCcw, Download } from 'lucide-react';
...
const PostUmrahManagement = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/post-umrah`, {
      ...
      console.error('Error fetching post-umrah data:', error.response?.data || error.message);
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      console.error('Error saving post-umrah data:', error.response?.data || error.message);
      if (error.response?.status === 400 && error.response?.data?.message?.includes('already exists')) {
        message.error('An entry with this ID already exists. Please use a different ID.');
      } else {
        message.error(error.response?.data?.message || 'Failed to save post-umrah data');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (ids.length > 1) {
        await axios.post(`${import.meta.env.VITE_BACKEND_URL_V2}/post-umrah/bulk-delete`,
          { ids },
          {
            headers: {
              Authorization: `Bearer ${token}`
        ...
        await axios.delete(`${import.meta.env.VITE_BACKEND_URL_V2}/post-umrah/${ids[0]}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          setUploadSuccess(`Successfully uploaded ${response.data.count} post-umrah entries`);
          setUploadError(null);
          fetchData();
          event.target.value = '';
        } catch (error) {
      ...
      <div className={`${sidebarOpen ? 'ml-72' : 'ml-20'}`}>
        ...
        <Modal
          title={
            <div className="flex items-center gap-2">
              <CheckCircle size={20} />
              <span>{editingId ? 'Edit Post-Umrah Entry' : 'Add New Post-Umrah Entry'}</span>
            </div>
          }
          open={modalVisible}
