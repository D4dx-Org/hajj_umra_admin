...
const TourManagement = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/tour`, {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      console.error('Error saving tour data:', error.response?.data || error.message);
      if (error.response?.status === 400 && error.response?.data?.message?.includes('already exists')) {
        message.error('An entry with this ID already exists. Please use a different ID.');
      } else {
        message.error(error.response?.data?.message || 'Failed to save tour data');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (ids.length > 1) {
        await axios.post(`${import.meta.env.VITE_BACKEND_URL_V2}/tour/bulk-delete`,
          { ids },
          {
            headers: {
              Authorization: `Bearer ${token}`
        ...
        await axios.delete(`${import.meta.env.VITE_BACKEND_URL_V2}/tour/${ids[0]}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const hasRequiredColumns = 'id' in firstRow && ('title_malayalam' in firstRow || 'titleMalayalam' in firstRow);

          if (!hasRequiredColumns) {
            setUploadError('Excel file must have required columns: id and title_malayalam');
            return;
          }
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (!row.id || !(row.title_malayalam || row.titleMalayalam)) {
              setUploadError(`Row ${rowNumber}: Missing required data. Each row must have id and title_malayalam.`);
              return;
            }
          }
          ...
          setUploadSuccess(`Successfully uploaded ${response.data.count} tour entries`);
          setUploadError(null);
          fetchData();
          event.target.value = '';
        } catch (error) {
      ...
      <div className={`${sidebarOpen ? 'ml-72' : 'ml-20'}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm divide-y divide-gray-200 table-fixed">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td colSpan={tourColumns.length} className="px-2 py-2 text-center text-gray-500">
                      Loading...
                    </td>
                  </tr>
        ...
        <Modal
          title={
            <div className="flex items-center gap-2">
              <MapPin size={20} />
              <span>{editingId ? 'Edit Tour Entry' : 'Add New Tour Entry'}</span>
            </div>
          }
          open={modalVisible}
