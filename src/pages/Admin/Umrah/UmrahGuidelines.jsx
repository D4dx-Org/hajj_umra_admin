...
const UmrahGuidelines = () => {
  ...
  const API_URL = `${import.meta.env.VITE_BACKEND_URL_V2}/umrah-umrah-guidelines`;
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "umrahguidelines_template.xlsx");
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "rules",
      render: (row) => (
        <span className="truncate" title={row.rules || '-'}>
          {row.rules || '-'}
        </span>
      ),
    },
      ...
      title: "doAndDonts",
      render: (row) => (
        <span className="truncate" title={row.doAndDonts || '-'}>
          {row.doAndDonts || '-'}
        </span>
      ),
    },
      ...
      title: "referenceLink",
      render: (row) => (
        <span className="truncate" title={row.referenceLink || '-'}>
          {row.referenceLink || '-'}
        </span>
      ),
    },
  ...
  return (
    <div>
      ...
      <p className="text-gray-900">{`${selectedItem.rules || 'Not provided'}`}</p>
    </div>
              <div>
      <label className="block text-sm font-medium text-gray-600 mb-1">doAndDonts</label>
      <p className="text-gray-900">{`${selectedItem.doAndDonts || 'Not provided'}`}</p>
    </div>
              <div>
        <label className="block text-sm font-medium text-gray-600 mb-1">referenceLink</label>
        {selectedItem.referenceLink ? (
          <div className="space-y-2">
            ...
            <div className="text-sm text-gray-600 break-all">
              <a href={selectedItem.referenceLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">
                {selectedItem.referenceLink}
              </a>
            </div>
          </div>
        ...
        >
          ...
          <TextArea rows={4} placeholder="Enter rules" />
        </Form.Item>
          <Form.Item name="doAndDonts" label="doAndDonts">
          <TextArea rows={4} placeholder="Enter doAndDonts" />
        </Form.Item>
          <Form.Item name="referenceLink" label="referenceLink">
          <Input placeholder="Enter referenceLink" />
        </Form.Item>
