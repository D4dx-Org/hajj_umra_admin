...
const DuasManagement = () => {
  ...
  const [selectedDuas, setSelectedDuas] = useState(null);
  const [showDetailModal, setShowDetailModal] = useState(false);

  // Handle row click to show details
  const handleRowClick = (duas, event) => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL_V2}/duas?limit=1000`,
        {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (
        ...
        message.error(
          error.response?.data?.message || "Failed to save duas data"
        );
      }
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (ids.length > 1) {
        await axios.post(
          `${import.meta.env.VITE_BACKEND_URL_V2}/duas/bulk-delete`,
          { ids },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        await axios.delete(
          `${import.meta.env.VITE_BACKEND_URL_V2}/duas/${ids[0]}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  // Get paginated data
  const paginatedDuasData = useMemo(() => {
    const startIndex = (pagination.current - 1) * pagination.pageSize;
    const endIndex = startIndex + pagination.pageSize;
    const paginated = filteredDuasData.slice(startIndex, endIndex);
    console.log(
      `Pagination: Page ${pagination.current}, showing ${paginated.length} of ${filteredDuasData.length} total items`
    );
    return paginated;
  }, [filteredDuasData, pagination.current, pagination.pageSize]);
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      setSelectedRows(paginatedDuasData.map((row) => row._id));
    } else {
      setSelectedRows([]);
    }
  };
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          setUploadSuccess(
            `Successfully uploaded ${response.data.count} duas entries`
          );
          setUploadError(null);
          fetchData();
          event.target.value = "";
  ...
  const duasColumns = [
    {
      ...
      title: (
        <input
          type="checkbox"
          checked={selectedRows.length === paginatedDuasData.length && paginatedDuasData.length > 0}
  ...
  return (
    <div>
      ...
      <div className={`${sidebarOpen ? "ml-72" : "ml-20"}`}>
            ...
            <table className="w-full text-sm divide-y divide-gray-200 table-fixed">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td
                      colSpan={duasColumns.length}
                      className="px-2 py-2 text-center text-gray-500"
                    >
                      Loading...
                    </td>
                  </tr>
                ) : paginatedDuasData.length === 0 ? (
                  <tr>
                    <td
                      colSpan={duasColumns.length}
                      className="px-2 py-2 text-center text-gray-500"
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-700">
                ...
                {Math.min(
                  ...
                  filteredDuasData.length
                )}{" "}
                of {filteredDuasData.length} duas entries
              </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {(() => {
                    const totalPages = Math.ceil(
                      filteredDuasData.length / pagination.pageSize
                    );
                  ...
                  <button
                    ...
                    disabled={
                      pagination.current >=
                      Math.ceil(filteredDuasData.length / pagination.pageSize)
                    }
                    className="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                    title="Next page"
                  >
        ...
        {/* Detail Modal */}
        {showDetailModal && selectedDuas && (
          <div className="fixed inset-0 bg-transparent backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">Duas Details</h2>
                  <button
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-900 font-medium">{selectedDuas.id}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Title (English)</label>
                        <p className="text-gray-900 font-medium">{selectedDuas.title || 'Not provided'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-gray-900">{selectedDuas.malayalamTitle || 'Not provided'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Urdu Title</label>
                        <p className="text-gray-900" dir="rtl">{selectedDuas.urduTitle || 'Not provided'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-gray-900 whitespace-pre-wrap">{selectedDuas.description || 'Not provided'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Malayalam Description</label>
                        <p className="text-gray-900 whitespace-pre-wrap">{selectedDuas.malayalamDescription || 'Not provided'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Urdu Description</label>
                        <p className="text-gray-900 whitespace-pre-wrap" dir="rtl">{selectedDuas.urduDescription || 'Not provided'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        {selectedDuas.images && selectedDuas.images.length > 0 ? (
                          <div className="grid grid-cols-4 gap-2 mt-2">
                            {selectedDuas.images.map((imageUrl, index) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-900 text-sm">
                          {selectedDuas.createdAt ? new Date(selectedDuas.createdAt).toLocaleString() : 'Not available'}
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Updated At</label>
                        <p className="text-gray-900 text-sm">
                          {selectedDuas.updatedAt ? new Date(selectedDuas.updatedAt).toLocaleString() : 'Not available'}
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-600 mb-1">Database ID</label>
                        <p className="text-gray-900 text-sm font-mono">{selectedDuas._id}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <Modal
          title={
            <div className="flex items-center gap-2">
              <BookOpen size={20} />
              <span>
                {editingId ? "Edit Duas Entry" : "Add New Duas Entry"}
              </span>
            </div>
          }
          open={modalVisible}
