...
const UmrahCamp = () => {
  ...
  const campColumns = [
    {
      ...
      title: (
        <input
          type="checkbox"
          checked={selectedRows.length === campData.length && campData.length > 0}
    ...
    },
    {
      key: 'maktab',
      title: 'Maktab',
      render: (row) => <span className="truncate" title={row.maktab || '-'}>{row.maktab || '-'}</span>
    },
    {
      key: 'malayalamMaktab',
      title: 'Ml Maktab',
      render: (row) => <span className="truncate" title={row.malayalamMaktab || '-'}>{row.malayalamMaktab || '-'}</span>
    },
    {
      key: 'urduMaktab',
      title: 'Ur Maktab',
      render: (row) => <span className="truncate" title={row.urduMaktab || '-'}>{row.urduMaktab || '-'}</span>
    },
    {
      key: 'zone',
      title: 'Zone',
      render: (row) => <span className="truncate" title={row.zone || '-'}>{row.zone || '-'}</span>
    },
    {
      key: 'malayalamZone',
      title: 'Ml Zone',
      render: (row) => <span className="truncate" title={row.malayalamZone || '-'}>{row.malayalamZone || '-'}</span>
    },
    {
      key: 'urduZone',
      title: 'Ur Zone',
      render: (row) => <span className="truncate" title={row.urduZone || '-'}>{row.urduZone || '-'}</span>
    },
    {
      key: 'country',
      title: 'Country',
      render: (row) => {
        ...
        return <span className="truncate" title={countryText}>{countryText}</span>;
      }
    },
    {
      key: 'poll',
      title: 'Poll',
      render: (row) => <span className="truncate" title={row.poll || 'N/A'}>{row.poll || 'N/A'}</span>
    },
    {
      key: 'road',
      title: 'Road',
      render: (row) => <span className="truncate" title={row.road || 'N/A'}>{row.road || 'N/A'}</span>
    },
    {
      key: 'tent',
      title: 'Tent',
      render: (row) => <span className="truncate" title={row.tent || 'N/A'}>{row.tent || 'N/A'}</span>
    },
    { 
      key: 'location', 
      title: 'Location',
  ...
  const sortOptions = [
    ...
    { value: 'malayalamMaktab-alpha-asc', label: 'Malayalam Maktab (A-Z)', field: 'malayalamMaktab', direction: 'asc', type: 'alpha' },
    { value: 'malayalamMaktab-alpha-desc', label: 'Malayalam Maktab (Z-A)', field: 'malayalamMaktab', direction: 'desc', type: 'alpha' },
    { value: 'urduMaktab-alpha-asc', label: 'Urdu Maktab (A-Z)', field: 'urduMaktab', direction: 'asc', type: 'alpha' },
    { value: 'urduMaktab-alpha-desc', label: 'Urdu Maktab (Z-A)', field: 'urduMaktab', direction: 'desc', type: 'alpha' },
    { value: 'zone-alpha-asc', label: 'Zone (A-Z)', field: 'zone', direction: 'asc', type: 'alpha' },
    { value: 'zone-alpha-desc', label: 'Zone (Z-A)', field: 'zone', direction: 'desc', type: 'alpha' },
    { value: 'malayalamZone-alpha-asc', label: 'Malayalam Zone (A-Z)', field: 'malayalamZone', direction: 'asc', type: 'alpha' },
    { value: 'malayalamZone-alpha-desc', label: 'Malayalam Zone (Z-A)', field: 'malayalamZone', direction: 'desc', type: 'alpha' },
    { value: 'urduZone-alpha-asc', label: 'Urdu Zone (A-Z)', field: 'urduZone', direction: 'asc', type: 'alpha' },
    { value: 'urduZone-alpha-desc', label: 'Urdu Zone (Z-A)', field: 'urduZone', direction: 'desc', type: 'alpha' },
    { value: 'country-alpha-asc', label: 'Country (A-Z)', field: 'country', direction: 'asc', type: 'alpha' },
    { value: 'country-alpha-desc', label: 'Country (Z-A)', field: 'country', direction: 'desc', type: 'alpha' },
    { value: 'poll-alpha-asc', label: 'Poll (A-Z)', field: 'poll', direction: 'asc', type: 'alpha' },
    { value: 'poll-alpha-desc', label: 'Poll (Z-A)', field: 'poll', direction: 'desc', type: 'alpha' }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/camp-umrah`);
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/country-umrah`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map(id => 
        axios.delete(`${import.meta.env.VITE_BACKEND_URL_V2}/camp-umrah/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleAddCamp = async () => {
    try {
      ...
      if (response.status === 201) {
        const updatedResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/camp-umrah`);
  ...
  const filteredCampData = useMemo(() => {
    ...
    if (lowerCaseSearch) {
      filtered = campData.filter((item) => {
        ...
        return (
          (item.maktab && item.maktab.toLowerCase().includes(lowerCaseSearch)) ||
          (item.malayalamMaktab && item.malayalamMaktab.toLowerCase().includes(lowerCaseSearch)) ||
          (item.urduMaktab && item.urduMaktab.toLowerCase().includes(lowerCaseSearch)) ||
          (item.zone && item.zone.toLowerCase().includes(lowerCaseSearch)) ||
          (item.malayalamZone && item.malayalamZone.toLowerCase().includes(lowerCaseSearch)) ||
          (item.urduZone && item.urduZone.toLowerCase().includes(lowerCaseSearch)) ||
          ...
          (item.malayalamRoad && item.malayalamRoad.toLowerCase().includes(lowerCaseSearch)) ||
          (item.urduRoad && item.urduRoad.toLowerCase().includes(lowerCaseSearch)) ||
          (item.tent && item.tent.toLowerCase().includes(lowerCaseSearch)) ||
          (item.malayalamTent && item.malayalamTent.toLowerCase().includes(lowerCaseSearch)) ||
          (item.urduTent && item.urduTent.toLowerCase().includes(lowerCaseSearch)) ||
  ...
  const handleFileUpload = async (event) => {
    try {
      ...
      setUploadSuccess(`Successfully uploaded ${response.data.inserted.length} camps`);
      if (response.data.errors && response.data.errors.length > 0) {
        setUploadError(`Some rows had issues: ${response.data.errors.join(', ')}`);
      } else {
        setUploadError(null);
      }
        ...
        {showAddForm && (
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium">Malayalam Maktab</label>
                <input
                  type="text"
                  value={newCamp.malayalamMaktab}
                  onChange={(e) => setNewCamp({ ...newCamp, malayalamMaktab: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="മക്തബ്"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, urduMaktab: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="مکتب"
                />
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium">Malayalam Zone</label>
                <input
                  type="text"
                  value={newCamp.malayalamZone}
                  onChange={(e) => setNewCamp({ ...newCamp, malayalamZone: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="മേഖല"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, urduZone: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="علاقہ"
                />
              </div>
              <div className="mb-4">
                ...
                <select
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, country: e.target.value, otherCountry: e.target.value ? '' : newCamp.otherCountry })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                >
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium">Other Country</label>
                <input
                  type="text"
                  value={newCamp.otherCountry}
                  onChange={(e) => setNewCamp({ ...newCamp, otherCountry: e.target.value, country: e.target.value ? '' : newCamp.country })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, malayalamPoll: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="പോൾ"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, urduPoll: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="پول"
                />
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium">Malayalam Road</label>
                <input
                  type="text"
                  value={newCamp.malayalamRoad}
                  onChange={(e) => setNewCamp({ ...newCamp, malayalamRoad: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="റോഡ്"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, urduRoad: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="سڑک"
                />
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium">Malayalam Tent</label>
                <input
                  type="text"
                  value={newCamp.malayalamTent}
                  onChange={(e) => setNewCamp({ ...newCamp, malayalamTent: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="കൂടാരം"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setNewCamp({ ...newCamp, urduTent: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="خیمہ"
                />
              </div>
            ...
            <table className="w-full text-sm divide-y divide-gray-200 table-fixed">
                  ...
                  {campColumns.map((column) => (
                    <th
                      ...
                      className={`px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                        ...
                        column.key === 'malayalamMaktab' ? 'w-20' :
                        column.key === 'urduMaktab' ? 'w-20' :
                        column.key === 'zone' ? 'w-16' :
                        column.key === 'malayalamZone' ? 'w-20' :
                        column.key === 'urduZone' ? 'w-20' :
                        column.key === 'country' ? 'w-24' :
                        column.key === 'poll' ? 'w-16' :
                        column.key === 'road' ? 'w-16' :
                        column.key === 'tent' ? 'w-16' :
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCampData.length === 0 ? (
                  <tr>
                    <td colSpan={campColumns.length} className="px-2 py-2 text-center text-gray-500">
                      No camps found
                    </td>
                  </tr>
                ) : (
                        ...
                        {campColumns.map((column) => (
                          <td
                            ...
                            className={`px-2 py-2 text-sm text-gray-900 ${
                              column.key === 'maktab' ? 'max-w-20 truncate' :
                              column.key === 'malayalamMaktab' ? 'max-w-20 truncate' :
                              column.key === 'urduMaktab' ? 'max-w-20 truncate' :
                              column.key === 'zone' ? 'max-w-16 truncate' :
                              column.key === 'malayalamZone' ? 'max-w-20 truncate' :
                              column.key === 'urduZone' ? 'max-w-20 truncate' :
                              column.key === 'country' ? 'max-w-24 truncate' :
                              column.key === 'poll' ? 'max-w-16 truncate' :
                              column.key === 'road' ? 'max-w-16 truncate' :
                              column.key === 'tent' ? 'max-w-16 truncate' :
                        ...
                        <tr>
                          <td colSpan={campColumns.length} className="p-4">
                            <div className="bg-white rounded-lg shadow p-4 mb-6 max-w-4xl mx-auto">
                              <h2 className="text-lg font-bold mb-4">Edit Camp</h2>
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleEditChange(row._id, "malayalamMaktab", e.target.value)
                                    }
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleEditChange(row._id, "urduMaktab", e.target.value)
                                    }
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleEditChange(row._id, "malayalamZone", e.target.value)
                                    }
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    value={row.urduZone || ""}
                                    onChange={(e) =>
                                      handleEditChange(row._id, "urduZone", e.target.value)
                                    }
