...
const BuildingKSA = ({ isOpen }) => {
  ...
  const buildingColumns = [
    ...
    { 
      key: 'name_malayalam', 
      title: 'Ml Name',
      render: (row) => <span className="truncate" title={row.name_malayalam || '-'}>{row.name_malayalam || '-'}</span>
    },
    { 
      key: 'name_urdu', 
      title: 'Ur Name',
      render: (row) => <span className="truncate" title={row.name_urdu || '-'}>{row.name_urdu || '-'}</span>
    },
    { 
      key: 'location', 
      title: 'Location',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/building`);
  ...
  const handleSaveEdit = async (row) => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_BACKEND_URL_V2}/building/${row._id}`,
        row,
        {
          headers: {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map(id => 
        axios.delete(`${import.meta.env.VITE_BACKEND_URL_V2}/building/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleAddBuilding = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL_V2}/building`,
        newBuilding,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 201) {
        const updatedResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL_V2}/building`);
    ...
    try {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const response = await axios.post(
                    `${import.meta.env.VITE_BACKEND_URL_V2}/building/bulk-upload`,
                    formData,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                  ...
                  {buildingColumns.map((column) => (
                    <th
                      ...
                      className={`px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                        ...
                        column.key === 'name_malayalam' ? 'w-20' :
                        column.key === 'name_urdu' ? 'w-20' :
                        ...
                        {buildingColumns.map((column) => (
                          <td
                            ...
                            className={`px-2 py-2 text-sm text-gray-900 ${
                              ...
                              column.key === 'name_malayalam' ? 'max-w-20 truncate' :
                              column.key === 'name_urdu' ? 'max-w-20 truncate' :
