...
//       render: (row) => <span className="truncate" title={row.maktabMalayalam || '-'}>{row.maktabMalayalam || '-'}</span>
//     },    { 
//       key: 'maktabUrdu', 
//       title: 'Maktab (Urdu)',
//       render: (row) => <span className="truncate" title={row.maktabUrdu || '-'}>{row.maktabUrdu || '-'}</span>
//     },    { 
//       key: 'zone', 
//       title: 'Zone',
...
//       render: (row) => <span className="truncate" title={row.zoneMalayalam || '-'}>{row.zoneMalayalam || '-'}</span>
//     },    { 
//       key: 'zoneUrdu', 
//       title: 'Zone (Urdu)',
//       render: (row) => <span className="truncate" title={row.zoneUrdu || '-'}>{row.zoneUrdu || '-'}</span>
//     },
//     {
//       key: 'country',
//       title: 'Country',
...
//       key: 'road', 
//       title: 'Road',
//       render: (row) => <span className="truncate" title={row.road || '-'}>{row.road || '-'}</span>
//     },    { 
//       key: 'roadMalayalam', 
//       title: 'Road (Malayalam)',
//       render: (row) => <span className="truncate" title={row.roadMalayalam || '-'}>{row.roadMalayalam || '-'}</span>
//     },    { 
//       key: 'roadUrdu', 
//       title: 'Road (Urdu)',
//       render: (row) => <span className="truncate" title={row.roadUrdu || '-'}>{row.roadUrdu || '-'}</span>
//     },    { 
//       key: 'tent', 
//       title: 'Tent',
//       render: (row) => <span className="truncate" title={row.tent || '-'}>{row.tent || '-'}</span>
//     },    { 
//       key: 'tentMalayalam', 
//       title: 'Tent (Malayalam)',
//       render: (row) => <span className="truncate" title={row.tentMalayalam || '-'}>{row.tentMalayalam || '-'}</span>
//     },    { 
//       key: 'tentUrdu', 
//       title: 'Tent (Urdu)',
//       render: (row) => <span className="truncate" title={row.tentUrdu || '-'}>{row.tentUrdu || '-'}</span>
//     },
//     {
//       key: 'location',
//       title: 'Location',
...
//                                 <div>
//                                   <label className="block text-sm font-medium">Other Country Name</label>
//                                   <input
//                                     type="text"
...
const Camp = ({ isOpen }) => {
  ...
  const handleEditClick = (row) => {
    const rowCopy = {
      ...row,
      country: row.country ? row.country._id : ''
    };
    setOriginalData(row);
    setEditingId(row._id);
    setCampData(campData.map(item =>
  ...
  const handleDownloadTemplate = () => {
    try {
      ...
      ws['!cols'] = [
        { wch: 25 }, { wch: 25 }, { wch: 25 }, { wch: 25 }, { wch: 20 },
            ...
            <table className="min-w-full full text-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCampData.map((row) => (
                  <React.Fragment key={row._id}>
                    <tr 
                      className={`${editingId === row._id ? 'bg-blue-50' : 'hover:bg-gray-50 cursor-pointer'}`}
         ...
         <div className="fixed inset-0 bg-transparent backdrop-blur-sm flex items-center justify-center z-50">
            ...
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold">Camp Details</h3>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Maktab Information</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Maktab (English)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.maktab || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Maktab (Malayalam)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.maktabMalayalam || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Maktab (Urdu)</label>
                    <p className="mt-1 text-gray-900" dir="rtl">{selectedRowData.maktabUrdu || 'N/A'}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Zone Information</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Zone (English)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.zone || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Zone (Malayalam)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.zoneMalayalam || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Zone (Urdu)</label>
                    <p className="mt-1 text-gray-900" dir="rtl">{selectedRowData.zoneUrdu || 'N/A'}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Location & Reference</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Country</label>
                    <p className="mt-1 text-gray-900">
                      {selectedRowData.otherCountry ? 
                        `${selectedRowData.otherCountry} (Other)` : 
                        (selectedRowData.country?.name || 'N/A')
                      }
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="mt-1 text-gray-900">
                      {selectedRowData.location?.lat && selectedRowData.location?.lng ? 
                        `${selectedRowData.location.lat}, ${selectedRowData.location.lng}` : 
                        'N/A'
                      }
                    </p>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-gray-900">{selectedRowData.ref?.title || selectedRowData.ref || 'N/A'}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Poll Information</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Poll (English)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.poll || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Poll (Malayalam)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.pollMalayalam || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Poll (Urdu)</label>
                    <p className="mt-1 text-gray-900" dir="rtl">{selectedRowData.pollUrdu || 'N/A'}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Road Information</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Road (English)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.road || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Road (Malayalam)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.roadMalayalam || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Road (Urdu)</label>
                    <p className="mt-1 text-gray-900" dir="rtl">{selectedRowData.roadUrdu || 'N/A'}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-semibold text-lg text-blue-600 border-b pb-2">Tent Information</h4>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Tent (English)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.tent || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Tent (Malayalam)</label>
                    <p className="mt-1 text-gray-900">{selectedRowData.tentMalayalam || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Tent (Urdu)</label>
                    <p className="mt-1 text-gray-900" dir="rtl">{selectedRowData.tentUrdu || 'N/A'}</p>
                  </div>
                </div>
              </div>
