...
const Camp = ({ isOpen }) => {
    ...
    const fetchCountries = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/camp/countries/list`
        );
  ...
  const CustomOption = ({ data, ...props }) => (
    ...
    >
      ...
      {data.arabicName && (
        <span className="text-gray-500">({data.arabicName})</span>
      )}
    </div>
  );
  ...
  const campColumns = [
    ...
    },
    {
      ...
      render: (row) => {
        if (row.otherCountry) {
          return <span className="truncate" title={row.otherCountry + " (Other)"}>{row.otherCountry} (Other)</span>;
        }
        return row.country ? (
          <div className="truncate" title={`${row.country.name} ${row.country.arabicName ? `(${row.country.arabicName})` : ''}`}>
            <span>{row.country.name}</span>
            {row.country.arabicName && (
              <span className="text-gray-500">({row.country.arabicName})</span>
            )}
          </div>
        ) : (
          "N/A"
    ...
    },
    {
      ...
      render: (row) => {
        ...
        let locationText = "";
        if (lat === undefined && lng === undefined) locationText = "N/A";
        else if (
          (lat === undefined || lat === "") &&
          lng !== undefined &&
          lng !== ""
        )
          locationText = `N/A, ${lng}`;
        else if (
          (lng === undefined || lng === "") &&
          lat !== undefined &&
          lat !== ""
        )
          locationText = `${lat}, N/A`;
        else
          locationText = `${lat ?? "N/A"}, ${lng ?? "N/A"}`;
    ...
    try {
      ...
      const dataToSave = {
        ...
        country:
          row.country === "others"
            ? null
            : row.country?._id || row.country || null,
      ...
      setCampData(
        campData.map((item) => (item._id === row._id ? response.data : item))
      );
  ...
  const handleEditClick = (row) => {
    ...
    setCampData(
      campData.map((item) => (item._id === row._id ? rowCopy : item))
    );
  };
  ...
  const handleFileUpload = async (event) => {
    try {
      ...
      if (
        ...
        response.data.failedRows.length > 0
      ) {
        setUploadError(
        ...
        setUploadSuccess(
          `Successfully uploaded ${response.data.count} camps (with some errors)`
        );
      } else {
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setNewCamp({
                    ...
                    country: value,
                    otherCountry:
                      value === "others" ? newCamp.otherCountry : "",
                  });
                }}
                className="mt-1 block w-full border border-gray-300 rounded-md p-2"
              >
              ...
              {newCamp.country === "others" && (
                <input
                  ...
                  onChange={(e) =>
                    setNewCamp({ ...newCamp, otherCountry: e.target.value })
                  }
            ...
            <div className="mb-4">
              ...
              <select
                value={newCamp.ref}
                onChange={(e) =>
                  setNewCamp({ ...newCamp, ref: e.target.value })
                }
                className="mt-1 block w-full border border-gray-300 rounded-md p-2"
              >
                    ...
                    {campColumns.map((column) => (
                      <th
                        ...
                        className={`px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                          ...
                          column.key === 'ref' ? 'w-24' :
                          column.key === 'road' ? 'w-20' :
                          column.key === 'tent' ? 'w-20' :
                          ...
                          {campColumns.map((column) => (
                            <td
                              ...
                              className={`px-2 py-2 text-sm text-gray-900 ${
                                column.key === 'country' ? 'max-w-32 truncate' :
